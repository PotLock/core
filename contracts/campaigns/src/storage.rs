...
impl Contract {
    ...
    pub fn storage_deposit(&mut self) -> U128 {
        let mut deposit = env::attached_deposit().as_yoctonear();
        ...
        if existing_mapping.is_none() {
            ...
            let required_deposit = storage_usage as u128 * env::storage_byte_cost().as_yoctonear();
            assert!(
    ...
    pub fn storage_withdraw(&mut self, amount: Option<U128>) -> U128 {
        ...
        if remainder > 0 {
            self.storage_deposits.insert(account_id.clone(), remainder);
            ...
            let refund = storage_usage as u128 * env::storage_byte_cost().as_yoctonear();
    ...
    pub(crate) fn verify_and_update_storage_balance(
        ...
        self.storage_deposits
            .insert(sender_id.clone(), new_storage_balance);
